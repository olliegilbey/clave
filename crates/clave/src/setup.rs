...
pub fn layout_kdl(binary: &str, wasm: &str) -> String {
    ...
    //
    // 22% is `BAR_TARGET_COLS / 200` against the 200-column reference viewport
    ...
    format!(
        "// GENERATED by `clave setup` — regenerate, don't hand-edit.\n\
         ...
         \x20       pane size=\"22%\" borderless=true {{\n\
...
pub fn launch_layout_kdl(
    ...
    // size="22%" not size=30: fixed panes refuse resizes — see layout_kdl.
...
mod tests {
    ...
    fn generated_bar_panes_are_percent_sized_not_fixed() {
        ...
        for kdl in [
            ...
            // 22% is BAR_TARGET_COLS (44) against S8 §3.4's 200-column
            // reference viewport, so a newborn bar is essentially on target
            // and does not flicker its way there.
            assert!(
                kdl.contains("size=\"22%\""),
