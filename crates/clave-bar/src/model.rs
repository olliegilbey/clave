...
use clave_types::{Agent, AgentSnapshot, OrderMode, RowHeight, Status, frecency_millis};
...
pub enum Effect {
    ...
    /// to the STORE (§6.6 Design B). Emitted by `bind_effects` from the
    /// elected instance, for a row whose pane is registered.
...
impl BarModel {
    ...
    /// The Alt+Enter open: mark the row in flight (↻) and ask the host.
