...
| 7b′ — birth for `clave open` / `add` tabs | not started, **CONFIRMED LIVE** | — | Observed 2026-07-31: the launch tab's bar pane sits at 28% (terminal-derived) while every dwell-opened tab sits at 27% (`BAR_BIRTH_PERCENT`, the fiction), so they rest one column apart and the difference is visible when switching tabs. | Those build one-shot layouts that bypass the template and run INSIDE zellij, so the TTY is the wrong width. Needs the bar to pass `get_tab_info().display_area_columns` down as a CLI argument. |
