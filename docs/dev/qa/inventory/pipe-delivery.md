...
**Guard today:** none (issue open; drop log still unconditional, latency never measured) — the twins are load-bearing as *diagnostics*, so never "fix" them silently; the gate is always the payload's own observable (the store bind).
