...
| 5b | **Card cells (hook side)** | six real `clave hook` events against the sandbox store: a permission `Notification`, `UserPromptSubmit`, three `Stop`s (one on a tail carrying an `Agent` launch, one on a background-command notification, one on the agent's own), `SessionEnd` | `wants` arrives from the notification's own words and leaves with the next prompt; the subagent mark rises on a fan-out, ignores a background command finishing beside it, clears on the notification naming its own `tool-use-id`, and stays down through `SessionEnd`; status follows each event; ≤1 store write per event, counted AFTER a PR-cache warm-up (`seq` is not a per-event counter — a stale cache makes the hook spawn `pr-sync`, a second writer landing off-schedule; FOOTGUNS). Fires every event through `ct.sh --hook`, never `clave hook` directly, and appends only to a `c85c` scenario transcript, guarded | #232's card cells — the seam two of its defects lived at |
