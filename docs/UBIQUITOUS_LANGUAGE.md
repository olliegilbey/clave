...
| **meter** | The statusLine channel: `clave statusline` wrapped around the user's status line command, fed Claude Code's statusLine JSON after every assistant message. The source of the battery's **count**, the model and the effort while it speaks (#245). **The hook yields to the meter**: once a row is metered, the hook's transcript-tail readers hold those three cells; a Stop or a rotation hands the floor back. A meter gone quiet degrades to the hook's tail. |
| **agent** | One record in the store / one entry in a snapshot. The data; a **row** is its rendering. |
| **frecency** | The default row-order score: Σ commitment count × `0.5^(age_hours / half_life_hours)` over a row's hour buckets. Ranks invested threads above whatever was merely touched last. `OrderMode::Recency` is the pure-ordinal alternative, still selectable (`clave order recency`). The Alt+a dir picker ranks store repos by the same score summed per repo — one number, two surfaces. |
| **hour bucket** | `unix_hour → commitment count`, where frecency's weight is banked. Hourly since v0.4.0 (day-keyed before: one UTC day was one age, so count alone ordered a day's rows and midnight halved everything at once; a day-era key reads as a 1972 hour, scores zero, and `backfill` re-derives the row from its transcript). Written twice per commitment, same doubling as `tab_order`/`commit_ord`: on the `AgentRecord` (uuid-keyed, survives dormancy) and on `Store.tab_buckets` (tab-keyed, covers terminal tabs). Pruned to the trailing 168 hours on every write — and, because that prune is lazy (it runs only on a row's next bump), a bucket outside the window scores ZERO in the bar at every dial. |
...
| Term | Means |
|---|---|
| **card** | A row's **four-line** rendering, the default since the four-line lock. Line 1 is status, chip and summary; line 2 is identity — provenance, repo, branch, PR, provider, model and effort; line 3 is state — subagents, tokens, the clock and what it wants (four cells, not five: the clock is ONE cell at three resolutions, seconds while the turn runs, then minutes and seconds to ten minutes, then coarse staleness — lock §4.4, amended 2026-09-14); line 4 is a hairline that closes the card. The four lines are one unit: one click target, one viewport slot. The separator does the job zebra parity used to. Locked in `docs/superpowers/specs/2026-09-08-triple-height-card-lock.md`. |
