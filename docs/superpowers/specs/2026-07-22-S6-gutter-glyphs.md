...
| **G7 — stripping the gutter reproduces the S5 line at equal text budget** | deleting the first `gutter_cols(cols, collapsed)` characters of S6's visible line at width `cols` yields exactly what S5's renderer produced for the same row at width **`cols − gutter_cols(cols, collapsed)`** — i.e. the *reference render must be given the same text budget the gutter leaves*, not the full `cols` (CodeRabbit 2026-07-22: comparing against a 0-gutter render at the full `cols` gives the reference a larger budget, so a long name truncates differently and the equality fails). Holding the text budget equal, S6 is provably additive to the text half. This is S5's own P6 (*"the gutter passes through verbatim"*) viewed from the other end; one of the two may be dropped if they prove redundant in review |
...
| you had to hand-edit and would rather not | **that is #40's follow-up**: `clave setup` should read `CLAVE_GLYPHS` and bake the key | file it, then Step 8 |
