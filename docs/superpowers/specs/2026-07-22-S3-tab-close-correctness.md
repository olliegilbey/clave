...
| `rows_render_as_two_blocks_live_above_dormant` (was `dormant_rows_sort_into_the_unified_recency_order`, renamed by #112) | `model.rs:2112` (assertion at `:2108`) | **mechanical** — `assert_eq!(d.glyph, Some(('◌', 90)))` → `Some(DORMANT_GLYPH)` | glyph const; its ordering assertions belong to S1 and #112 and stay |
