...
| **frecency** | The default row-order score: Σ commitment count × `0.5^(age_hours / half_life_hours)` over a row's hour buckets. Ranks invested threads above whatever was merely touched last. `OrderMode::Recency` is the pure-ordinal alternative, still selectable (`clave order recency`). |
